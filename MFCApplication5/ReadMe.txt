================================================================================
    MFC 程式庫：MFCApplication5 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 MFCApplication5 應用程式。這個應用程式不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案包含一份摘要，簡要說明構成 MFCApplication5 應用程式的
所有檔案，它們個別的內容。
...
MFCApplication5.h
    這是應用程式的主要標頭檔。
    它包含了特定標頭 (包括 Resource.h)，並宣告 CMFCApplication5App 應用程式類別。

MFCApplication5.cpp
    這是含有應用程式類別 CMFCApplication5App 的主要應用程式原始程式檔。
...
res\MFCApplication5.ico
    這是用來做為應用程式圖示的圖示檔。這個圖示是包含在主要資源檔 MFCApplication5.rc 中。
...
MFCApplication5Dlg.h、MFCApplication5Dlg.cpp - 對話方塊
    這些檔案包含您的 CMFCApplication5Dlg 類別。這個類別會定義應用程式主對話方塊的行為。對話方塊的範本是在可於 Microsoft Visual C++ 編輯的 MFCApplication5.rc 中。
...
StdAfx.h, StdAfx.cpp
    這些檔案是用來建置名為 MFCApplication5.pch 的先行編譯標頭 (PCH) 檔，以及名為 StdAfx.obj 的先行編譯型別檔。
